...
public class ListContainerComponent implements ContainerComponent {
	...
	public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick, AbstractContainerScreen<?> screen) {
		for(int i = 0;i<size;i++) {
			components[i].render(guiGraphics, mouseX, mouseY, partialTick, screen);
		}
	}
	
	@Override
	public void postRender(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick,
			AbstractContainerScreen<?> screen) {
		for(int i = 0;i<size;i++) {
			components[i].postRender(guiGraphics, mouseX, mouseY, partialTick, screen);
		}
	}
