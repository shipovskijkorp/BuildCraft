...
//        BCCoreRecipes.init();
    ...
    public void attachItemCapabilities(AttachCapabilitiesEvent<ItemStack> event) {
        if (!BCCoreItems.FRAGILE_FLUID_SHARD.isPresent()
            || event.getObject().getItem() != BCCoreItems.FRAGILE_FLUID_SHARD.get()) {
            return;
        }
        ItemFragileFluidContainer.FragileFluidHandler handler =
            new ItemFragileFluidContainer.FragileFluidHandler(event.getObject());
        event.addCapability(
            ResourceLocation.fromNamespaceAndPath(MODID, "fragile_fluid_shard"),
            handler
        );
    ...
    public static class ClientModEvents
    {
        ...
        public static void registerGeometryLoaders(RegisterGeometryLoaders event) {
            event.register("fragile_fluid_container", FragileFluidContainerModel.Loader.INSTANCE);
        }
