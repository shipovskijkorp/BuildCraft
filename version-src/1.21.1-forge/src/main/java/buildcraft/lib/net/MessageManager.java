...
public class MessageManager {
    ...
    public static final int PROTOCOL_VERSION = Math.max(
        1,
        BuildCraftTarget.NETWORK_PROTOCOL.hashCode() & Integer.MAX_VALUE
    );
    ...
    static class PerModHandler {
        ...
        PerModHandler(IBuildCraftMod module) {
            ...
            this.netWrapper = ChannelBuilder
                .named(ResourceLocation.fromNamespaceAndPath(module.getModId(), "channel"))
    ...
    static class PerMessageInfo<I> {
        ...
        @Nullable
        BiConsumer<I, Supplier<CustomPayloadEvent.Context>> clientHandler, serverHandler;
    ...
    public static synchronized <I> void registerMessageClass(IBuildCraftMod module, Class<I> messageClass,
    	BiConsumer<I, Supplier<CustomPayloadEvent.Context>> messageHandler,
    ...
    public static <I> void setHandler(Class<I> messageClass,
    	BiConsumer<I, Supplier<CustomPayloadEvent.Context>> messageHandler, Dist side) {
    ...
    private static <I> void postInitSingle(PerModHandler handler, PerMessageInfo<I> info) {
        ...
        // Forge 52 indexes SimpleChannel registrations by message class, so registering the same
        // class once for PLAY_TO_CLIENT and again for PLAY_TO_SERVER throws an
        // "already registered" exception. Register every message class exactly once and select
        // the sided handler from the actual receiving context instead.
        handler.netWrapper.messageBuilder(msgClass, id++)
            .encoder((message, buffer) -> info.enCoder.accept(message, buffer))
            .decoder(buffer -> info.deCoder.apply(buffer))
            .consumerNetworkThread(wrapHandlers(info, msgClass))
            .add();
        if (DEBUG) {
    ...
    private static <I> BiConsumer<I, CustomPayloadEvent.Context> wrapHandlers(
        PerMessageInfo<I> info, Class<I> messageClass
    ) {
        BiConsumer<I, CustomPayloadEvent.Context> client =
            wrapHandler(info.clientHandler, messageClass, Boolean.TRUE);
        BiConsumer<I, CustomPayloadEvent.Context> server =
            wrapHandler(info.serverHandler, messageClass, Boolean.FALSE);
        return (message, context) -> (context.isClientSide() ? client : server).accept(message, context);
    }

    private static <I> BiConsumer<I, CustomPayloadEvent.Context> wrapHandler(
        BiConsumer<I, Supplier<CustomPayloadEvent.Context>> messageHandler, Class<I> messageClass,
        @Nullable Boolean expectedClientSide
    ) {
        if (messageHandler == null) {
            return (message, context) -> {
                if (context.isServerSide()) {
                    ServerPlayer player = context.getSender();
                    String playerName = player == null ? "unknown client" : player.getGameProfile().getName();
                    BCLog.logger.warn("[lib.messages] " + playerName + " sent an invalid message "
                        + messageClass + ", when they should only receive it");
                } else {
                    BCLog.logger.error("Received message " + messageClass
                        + " on the client, when it should only be received on the server");
                }
        ...
        return (message, context) -> {
            if (expectedClientSide != null && context.isClientSide() != expectedClientSide.booleanValue()) {
                BCLog.logger.warn("Ignoring message " + messageClass + " received on the wrong logical side");
                context.setPacketHandled(true);
                return;
            }
            if (context.isClientSide()) {
                context.enqueueWork(() -> DistExecutor.unsafeRunWhenOn(
                    Dist.CLIENT, () -> () -> messageHandler.accept(message, () -> context)
                ));
            } else {
                ServerPlayer player = context.getSender();
                ...
                context.enqueueWork(() -> messageHandler.accept(message, () -> context));
            }
            context.setPacketHandled(true);
        };
    }
...
//        getSimpleNetworkWrapper(message).sendTo(message, player.connection.connection, NetworkDirection.PLAY_TO_CLIENT);
    ...
    public static void sendToAllWatching(Object message, LevelChunk levelChunk) {
    	getSimpleNetworkWrapper(message).send(message, PacketDistributor.TRACKING_CHUNK.with(levelChunk));
    }
