...
public class MessageUpdateTile {
    ...
    public static final BiConsumer<MessageUpdateTile, Supplier<CustomPayloadEvent.Context>> HANDLER =
        (message, supplier) -> {
    ...
    private static void handle(MessageUpdateTile message, CustomPayloadEvent.Context context) {
        FriendlyByteBuf payloadBuffer = null;
        boolean clientSide = context.isClientSide();
        try {
            Level level;
            ...
            if (!clientSide) {
                ...
                level = sender.level();
                if (!level.hasChunkAt(message.pos)) return;
            } else {
                ...
                level = net.minecraftforge.fml.loading.FMLEnvironment.dist.isClient()
                    ? MessageUpdateTileClientHandler.getClientLevel(listener)
                    : null;
                if (level == null || !level.hasChunkAt(message.pos)) return;
            }
            ...
            if (!clientSide) {
                if (tile instanceof TileBC_Neptune bcTile) {
                    if (!bcTile.canInteractWith(sender)) return;
                } else {
                    double x = message.pos.getX() + 0.5D;
                    double y = message.pos.getY() + 0.5D;
                    double z = message.pos.getZ() + 0.5D;
                    if (sender.distanceToSqr(x, y, z) > MAX_INTERACTION_DISTANCE_SQR) return;
                }
            }
