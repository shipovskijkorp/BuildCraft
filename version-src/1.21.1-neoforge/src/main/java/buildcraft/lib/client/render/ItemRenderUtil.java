...
/*        GL11.glPushMatrix();
    ...
    public static void renderItemStack(double x, double y, double z, ItemStack stack, int stackCount, int lightc,
        ...
        try {
            ...
            category.setDetail("Item ID", () -> "" + BuiltInRegistries.ITEM.getKey(stack.getItem()));
//            category.setDetail("Item Meta", () -> "" + stack.());
            category.setDetail("Item Components", () -> "" + stack.getComponentsPatch());
...
//            Minecraft.getInstance().getModelManager().tex(InventoryMenu.BLOCK_ATLAS);
    ...
    private static void setupModelOffsetRandom(ItemStack stack) {
        ...
        if (stack.isEmpty()) {
            ...
            ResourceLocation regName = ResourceLocation.parse(stack.getItem().getDescriptionId());
            if (regName == null) {
                seed = 127;
            } else {
                int regNameSeed = regName.getNamespace().hashCode() ^ regName.getPath().hashCode();
                seed = (regNameSeed & 0x7F_FF_FF_FF) | (((long) stack.getComponentsPatch().hashCode()) << 32);
            }
        }
