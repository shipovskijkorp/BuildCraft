...
public enum FakePlayerProvider {
    ...
    public FakePlayerBC getFakePlayer(ServerLevel world, GameProfile profile, BlockPos pos) {
        ...
        Map<GameProfile, FakePlayerBC> worldPlayers = playersByWorld.computeIfAbsent(world, ignored -> new HashMap<>());
        FakePlayerBC player = worldPlayers.computeIfAbsent(profile, p -> new FakePlayerBC(world, p));
        player.setPos(pos.getX(), pos.getY(), pos.getZ());

        // Constructing a ServerPlayer with the profile of an online player makes vanilla's shared
        // PlayerAdvancements instance point at the connection-less fake player. Restore the real player
        // immediately, otherwise the next recipe reward can try to send a packet through a null connection.
        if (profile.getId() != null) {
            ServerPlayer onlinePlayer = world.getServer().getPlayerList().getPlayer(profile.getId());
            if (onlinePlayer != null && onlinePlayer != player) {
                onlinePlayer.getAdvancements().setPlayer(onlinePlayer);
            }
        }
        return player;
    }
