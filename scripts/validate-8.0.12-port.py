#!/usr/bin/env python3
"""Offline structural validation for the BuildCraft 8.0.12 Forge 1.20.1 port."""
...
def main() -> None:
    ...
    for rel in required:
        if not (root / rel).is_file():
            fail(f"missing 8.0.12 port file: {rel}")
    ...
    for rel, needles in checks.items():
        ...
        for needle in needles:
            if needle not in text:
                fail(f"missing expected 8.0.12 invariant {needle!r} in {rel}")

    print(
        f"BuildCraft 8.0.12 port layout OK: {len(java_files)} Java files, "
