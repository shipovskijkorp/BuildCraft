...
public enum DebugRenderHelper implements IDetachedRenderer {
    ...
    public static void renderAABB(PoseStack pose, Matrix4f matrix, BufferBuilder bb, AABB aabb, int colour) {
    	if(s == null) s = Minecraft.getInstance().getModelManager().getAtlas(InventoryMenu.BLOCK_ATLAS).getSprite(new ResourceLocation("minecraft", "block/quartz_block_top"));//TODO change to white
    	pose.pushPose();
