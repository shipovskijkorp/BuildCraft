...
public final class MjReceiverEnergyStorage implements IEnergyStorage {
    ...
    public int getEnergyStored() {
        ...
        return (int) Math.min(Integer.MAX_VALUE, MjAPI.getFeConversion().microMjToFe(readable.getStored()));
    }

    @Override
    public int getMaxEnergyStored() {
        if (readable != null) {
            return (int) Math.min(Integer.MAX_VALUE, MjAPI.getFeConversion().microMjToFe(readable.getCapacity()));
        }
        return (int) Math.min(Integer.MAX_VALUE, MjAPI.getFeConversion().microMjToFe(receiver.getPowerRequested()));
    }
}
