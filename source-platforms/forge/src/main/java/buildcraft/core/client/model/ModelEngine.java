...
//	protected final TextureAtlasSprite tex;
	...
	public static void init(BakedModel model) {
		...
		//?} else {
		/*?
		back_faces = faces.stream().filter((a) -> a.getSprite().contents().name().getPath().equals("blocks/engine/wood/back")).toList();
		side_faces = faces.stream().filter((a) -> a.getSprite().contents().name().getPath().equals("blocks/engine/wood/side")).toList();
		trunk_faces = faces.stream().filter((a) -> a.getSprite().contents().name().getPath().equals("blocks/engine/trunk")).toList();
		?*/
		//?}
