...
public class JsonUtil {
    ...
    private static JsonSyntaxException failAndListFluids(String name) {
        Set<ResourceLocation> knownFluids = BuiltInRegistries.FLUID.keySet();
    ...
    public static final JsonDeserializer<ItemStack> ITEM_STACK_DESERIALIZER = (json, type, ctx) -> {
        if (json.isJsonPrimitive() && json.getAsJsonPrimitive().isString()) {
            ...
            if (obj.has("data")) {
                String serializedData = obj.get("data").isJsonPrimitive()
                    ? obj.get("data").getAsString()
                    : obj.get("data").toString();
                try {
                    data = TagParser.parseTag(serializedData);
                } catch (CommandSyntaxException e) {
                    throw new JsonSyntaxException("Invalid item data tag for '" + id + "': " + serializedData, e);
                }
...
//                meta = JsonUtil.getInt(obj, "meta");
     ...
     * for a {@link TranslatableContents}, or the prefix plus "_raw" for raw text. */
    ...
    //BUG
    public static final Codec<Pair<ParameterPoint, ResourceKey<Biome>>> BIOME_CODEC = Codec.pair(ParameterPoint.CODEC, ResourceKey.codec(Registries.BIOME));
    public static final Codec<String> PARA_TO_STRING_CODEC = ParameterPoint.CODEC.xmap(a -> a.toString(), null);
    public static final Codec<Map<String, ResourceKey<Biome>>> BIOME_MAP_CODEC = Codec.unboundedMap(Codec.STRING, ResourceKey.codec(Registries.BIOME));
...
//    	return ParameterPoint.
    ...
    public static JsonElement codeBiomePair(Pair<ParameterPoint, ResourceKey<Biome>> pair) {
		return BIOME_CODEC.encodeStart(JsonOps.INSTANCE, pair).getOrThrow(s -> {
            BCLog.logger.debug("BuildCraft:JsonUtil:Can not codec " + pair + "\n"
    ...
    public static Pair<ParameterPoint, ResourceKey<Biome>> decodeBiomePair(String debugInfo ,JsonElement json) {
		return BIOME_CODEC.parse(JsonOps.INSTANCE, json).getOrThrow(s -> {
            BCLog.logger.debug("BuildCraft:JsonUtil:Can not decodec " + debugInfo + "\n"
                + "with error :" + s + "\n this should not happen.");
            return new IllegalStateException(s);
        });
    }
