...
public final class MjReceiverEnergyStorage implements IEnergyStorage {
    ...
    public int receiveEnergy(int maxReceive, boolean simulate) {
        ...
        long offeredMj = MjAPI.getFeConversion().feToMicroMj(maxReceive);
    ...
    @Override public boolean canReceive() { return MjAPI.isFeAutoConversionEnabled() && receiver.canReceive(); }
    @Override public int getEnergyStored() { return readable == null ? 0 : (int)Math.min(Integer.MAX_VALUE, MjAPI.getFeConversion().microMjToFe(readable.getStored())); }
    @Override public int getMaxEnergyStored() { return readable == null ? (int)Math.min(Integer.MAX_VALUE, MjAPI.getFeConversion().microMjToFe(receiver.getPowerRequested())) : (int)Math.min(Integer.MAX_VALUE, MjAPI.getFeConversion().microMjToFe(readable.getCapacity())); }
}
