...
public final class PipeDefinition {
    ...
    private PipeType apiType;
    private ResourceLocation runtimeComponentId;
//    public final int tex_id = BCTransportPipes.tex_int++; 
    ...
    public PipeDefinition(PipeDefinitionBuilder builder) {
        ...
        for (int i = 0; i < textures.length; i++) {
            textures[i] = ResourceLocation.parse(builder.texturePrefix+builder.textureSuffixes[i]);
        }
    ...
    /** Creates an internal runtime definition for a public API2 variant while reusing stable built-in behaviour. */
    public static PipeDefinition apiVariant(PipeType type, PipeDefinition base) {
        if (type == null) throw new NullPointerException("type");
        if (base == null) throw new NullPointerException("base");
        PipeDefinition variant = new PipeDefinition(type.id(), base, type);
        return variant;
    }

    private PipeDefinition(ResourceLocation identifier, PipeDefinition base, PipeType type) {
        this.identifier = identifier;
        this.textures = base.textures.clone();
        this.logicConstructor = base.logicConstructor;
        this.logicLoader = base.logicLoader;
        this.flowType = base.flowType;
        this.itemTextureTop = base.itemTextureTop;
        this.itemTextureCenter = base.itemTextureCenter;
        this.itemTextureBottom = base.itemTextureBottom;
        this.itemModelTop = base.itemModelTop;
        this.itemModelCenter = base.itemModelCenter;
        this.itemModelBottom = base.itemModelBottom;
        this.canBeColoured = type.colorable();
        this.colourType = base.colourType;
        this.apiType = type;
        this.runtimeComponentId = base.runtimeComponentId;
    }



    /** API2 type paired with this internal rendering/runtime definition. */
    public PipeType getApiType() {
        return apiType;
    }

    public void setApiType(PipeType apiType) {
        if (this.apiType != null && this.apiType != apiType) {
            throw new IllegalStateException("Pipe definition already linked to another API2 type: " + identifier);
        }
        this.apiType = apiType;
    }

    public ResourceLocation getRuntimeComponentId() {
        return runtimeComponentId;
    }

    public void setRuntimeComponentId(ResourceLocation runtimeComponentId) {
        this.runtimeComponentId = runtimeComponentId;
    }
